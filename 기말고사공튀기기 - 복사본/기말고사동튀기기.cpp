...
void ball_bar_collison() // 공과 바의 충돌 구현
{
	...
	float bar_y[2];
	float bar_collison;
	int collison_flag; // 충돌 기준
	
	collison_flag = bar_col / radius; // 바의 길이와 공의 반지름을 나누어서 바의 타점 개수를 넣는다. 70 / 10 = 7 -> 타점 7개 입력

	for (int i = 0; i < collison_flag * 2; i++) { // *2 는 bar_col은 마우스위치에서 왼쪽과 오른쪽으로 바가 벌어진 만큼이기 때문에 양쪽 다 계산 하려면 *2 필요
		bar_x[i] = (m_x - bar_col) + (i * bar_col/collison_flag); // (바의 x축 왼쪽) + (타점 간격만큼의 x축) ex) i = 1일 때, (바의 x축 왼쪽) + 1 * 70 / 7 = 10 만큼을 +해서 저장
	}

	bar_y[0] = m_y + bar_row / 2; // 바의 위쪽 위치
	bar_y[1] = m_y - bar_row / 2; // 바의 아래쪽 위치

	for (int i = 0; i < collison_flag * 2; i++) // 바의 윗부분중 좌상단 중심위 우상단부분만 계산(전체 계산하면 오히려 더 버그 걸림)
	{
		distance_x = ball_center.x - bar_x[i];
		distance_y = ball_center.y - bar_y[i % 2];
	...
	bar_x[2] = (m_x); // 바의 중심 위
	bar_x[3] = (m_x - bar_col); // 바의 왼쪽 아래
	bar_x[4] = (m_x + bar_col); // 바의 오른쪽 아래
	bar_x[5] = (m_x); // 바의 중심 아래
	bar_y[0] = m_y + bar_row / 2; // 바의 위쪽 위치
	bar_y[1] = m_y - bar_row / 2; // 바의 아래쪽 위치
	
	for (int i = 0; i < 6; i++) // 바의 윗부분중 좌상단 중심위 우상단부분만 계산(전체 계산하면 오히려 더 버그 걸림)
	{
		distance_x = ball_center.x - bar_x[i];
		distance_y = ball_center.y - bar_y[i / 3];
		distance = sqrt(distance_x * distance_x + distance_y * distance_y);
		if (distance <= radius)
		{
			ball_velocity.y *= -1.0;
